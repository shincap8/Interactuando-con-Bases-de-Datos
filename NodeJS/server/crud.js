...
module.exports.CrearUsuarioDemo = function (callback) {
    ...
    Usuario.insertMany(arr, function (error, docs) {
        if(error){
            ...
            callback(null, "Los usuarios se han registrado correctamente.")
        }
    });
}
...
module.exports.CrearEventoDemo = function (callback) {
    ...
    Evento.insertMany(arr, function (error, docs) {
        if (error) {
            callback(error.message)
        } else {
            callback(null, "Los eventos se han registrado correctamente.")
        }
    });
}
